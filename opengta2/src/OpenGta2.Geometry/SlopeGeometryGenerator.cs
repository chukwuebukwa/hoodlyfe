...
internal static class SlopeGeometryGenerator
{
    // rotation of cube
    private static readonly Matrix _matrixRotateUp = Matrix.Identity;
    private static readonly Matrix _matrixRotateRight = Matrix.CreateRotationZ(MathF.PI / 2) * Matrix.CreateTranslation(1, 0, 0);
    private static readonly Matrix _matrixRotateDown = Matrix.CreateRotationZ(MathF.PI) * Matrix.CreateTranslation(1, 1, 0);
    private static readonly Matrix _matrixRotateLeft = Matrix.CreateRotationZ(-MathF.PI / 2) * Matrix.CreateTranslation(0, 1, 0);

    // face to cube
    private static readonly Matrix _matrixFaceToCubeLeft = Matrix.CreateRotationX(-MathF.PI / 2) *
                                             Matrix.CreateRotationZ(MathF.PI / 2) *
                                             Matrix.CreateTranslation(new Vector3(0, 0, 1));

    private static readonly Matrix _matrixFaceToCubeRight = Matrix.CreateRotationX(-MathF.PI / 2) *
                                              Matrix.CreateRotationZ(-MathF.PI / 2) *
                                              Matrix.CreateTranslation(new Vector3(1, 1, 1));

    private static readonly Matrix _matrixFaceToCubeTop = Matrix.CreateRotationX(-MathF.PI / 2) *
                                            Matrix.CreateRotationZ(MathF.PI) *
                                            Matrix.CreateTranslation(new Vector3(1, 0, 1));

    private static readonly Matrix _matrixFaceToCubeBottom = Matrix.CreateRotationX(-MathF.PI / 2) *
                                               Matrix.CreateTranslation(new Vector3(0, 1, 1));

    private static readonly Matrix _matrixFaceToCubeLid = Matrix.CreateTranslation(new Vector3(0, 0, 1));
    ...
    private static Matrix GetRotationMatrix(Rotation rotation)
    {
        ...
        {
            Rotation.Rotate90 => _matrixRotateRight,
            Rotation.Rotate180 => _matrixRotateDown,
            Rotation.Rotate270 => _matrixRotateLeft,
            Rotation.Rotate0 => _matrixRotateUp,
            _ => _matrixRotateUp
        };
    }
    ...
    private static void GetBlockFaces(ref BlockInfo block, Rotation rotation, out FaceInfo top, out FaceInfo bottom, out FaceInfo left, out FaceInfo right)
    {
        ...
        {
            return ref block.GetFace(Rotate(face, rotation));
        }
    }
    ...
    private static float GetShade(int num)
    {
    ...
    private static GeometryVertex GetVertexOnFace(Vector3 point, Vector2 uv, ref FaceInfo face, Face direction, Matrix translation, Rotation blockRotation, float shading = -1)
    {
        var matrix = direction switch
        {
            Face.Left => _matrixFaceToCubeLeft,
            Face.Right => _matrixFaceToCubeRight,
            Face.Top => _matrixFaceToCubeTop,
            Face.Bottom => _matrixFaceToCubeBottom,
            Face.Lid => _matrixFaceToCubeLid,
    ...
    /// <param name="shading">The shading of the face. Value should range between 0-1. If the value is negative, the value is computed based on teh direction of the face.</param>
    /// <returns>The created vertex.</returns>
    private static GeometryVertex GetVertex(Vector3 position, Vector2 uv, ref FaceInfo face, Face direction, Matrix translation, Rotation blockRotation, float shading = -1)
    {
        ApplyDefaultShading(ref shading, ref face, direction, blockRotation);

        return new GeometryVertex(Vector3.Transform(position, translation), MapUv(ref face, direction == Face.Lid ? blockRotation : Rotation.Rotate0, uv.X, uv.Y), shading);
    }
    ...
    private static void AddIndex(Buffers buffers, ref FaceInfo face, float drawOrder, int index, bool oppositeFlat = false)
    {
        if (face.Flat || oppositeFlat)
            buffers.FlatIndices.Add((drawOrder, index));
        else
            buffers.Indices.Add(index);
    }
    ...
    private static void SlopeDiagonalSlope(ref BlockInfo block, float drawOrder, Rotation blockRotation, Matrix translationMatrix, Buffers buffers)
    {
        ...
        if (block.Lid.TileGraphic == 1023)
        {
            ...
            if (left.TileGraphic != 0)
            {
                ...
                buffers.Vertices.Add(GetVertex(new Vector3(0, 1, 1), new Vector2(1, 0), ref left, Face.Left, translation, blockRotation));
                buffers.Vertices.Add(GetVertex(new Vector3(0, 1, 0), new Vector2(1, 1), ref left, Face.Left, translation, blockRotation));
                buffers.Vertices.Add(GetVertex(new Vector3(0, 0, 0), new Vector2(0, 1), ref left, Face.Left, translation, blockRotation));

                AddIndex(buffers, ref left, drawOrder, start + 0);
                AddIndex(buffers, ref left, drawOrder, start + 1);
                AddIndex(buffers, ref left, drawOrder, start + 2);
            }
            ...
            if (block.Bottom.TileGraphic != 0)
            {
                ...
                buffers.Vertices.Add(GetVertex(new Vector3(0, 1, 1), new Vector2(0, 0), ref bottom, Face.Bottom, translation, blockRotation));
                buffers.Vertices.Add(GetVertex(new Vector3(1, 1, 0), new Vector2(1, 1), ref bottom, Face.Bottom, translation, blockRotation));
                buffers.Vertices.Add(GetVertex(new Vector3(0, 1, 0), new Vector2(1, 0), ref bottom, Face.Bottom, translation, blockRotation));
        ...
        else
        {
            ...
            AddSimpleFace(ref left, drawOrder, Face.Left, buffers, translationMatrix);
            AddSimpleFace(ref bottom, drawOrder, Face.Bottom, buffers, translationMatrix);
            ...
            if (diagonal.TileGraphic != 0)
            {
                ...
                buffers.Vertices.Add(GetVertex(new Vector3(1, 1, 1), new Vector2(0, 0), ref diagonal, Face.Right, translation, blockRotation, diagonalShade));
                buffers.Vertices.Add(GetVertex(new Vector3(0, 0, 1), new Vector2(1, 0), ref diagonal, Face.Right, translation, blockRotation, diagonalShade));
                buffers.Vertices.Add(GetVertex(new Vector3(1, 0, 0), new Vector2(0.5f, 1), ref diagonal, Face.Right, translation, blockRotation, diagonalShade));
    ...
    internal static void Push(ref BlockInfo block, Vector3 offset, GeometryBuffer<GeometryVertex> vertices, GeometryBuffer<int> indices, GeometryBuffer<(float DrawOrder, int Index)> flatIndices)
    {
    ...
    private record struct Buffers(GeometryBuffer<GeometryVertex> Vertices, GeometryBuffer<int> Indices, GeometryBuffer<(float DrawOrder, int Index)> FlatIndices);
}