...
- `python3 scripts/import-lpc-catalog.py` copies the curated browser editor catalog into `public/assets/custom/lpc-catalog/`.
- `/creator.html` is now an LPC recipe editor. `USE IN GAME` stores the serialized LPC recipe on the player's appearance.
...
- The first editor catalog is curated, not the full LPC universe: male base, several faces, hairstyles, colorable tops, pants, shoes, and shared color variants.
- Hair color is applied by NOCK0's browser composer as a tint over fixed LPC hair sheets; clothing uses LPC color variant files where available.
- Custom fixed-color clothing can live beside LPC layers. `Smiley Tee` is generated from an LPC shirt base with a baked front-facing chest mark, so the top color swatches are disabled for that item.
