...
- **Identity today** is a client-chosen guest name plus a first-run onboarding overlay. The overlay can use Privy's vanilla email OTP flow and send a Privy access token with `player.spawn`; the room verifies it when `PRIVY_APP_SECRET` is configured and keeps a session-local identity map. There is still no durable account table, reconnection, or pre-join `onAuth`, and CORS reflects any origin (`server/index.ts:10`).
...
- Client: the current middle-ground implementation keeps the title/login/creator overlay over the running game. A first-time client joins as a spectator so the district can render behind the window, then completes Privy email OTP or guest mode before sending `player.spawn`. The longer-term login-gated version should move the Privy access token into the top-level `joinOrCreate` options before room entry.
- Server: the current room verifies `player.spawn.auth` via `PrivyClient.utils().auth().verifyAccessToken()` when `PRIVY_APP_SECRET` is configured and stores a session-local identity. The Phase 1 target remains a proper `DistrictRoom.onAuth` hook that verifies before `onJoin`, resolves the durable account, and binds the wallet address. Guest play remains allowed; guest-to-wallet upgrade preserves progress via the account record, not the session.
